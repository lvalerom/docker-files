...
launch_xvfb() {
    ...
    until xdpyinfo -display ${DISPLAY} > /dev/null 2>&1 ; do
        ...
        if [ ${loopCount} -gt ${timeout} ] ; then
            echo "${LOG_ERROR} xvfb failed to start."
            exit 1
        fi
    done
}
...
launch_window_manager() {
    ...
    until wmctrl -m > /dev/null 2>&1 ; do
        ...
        sleep 1
        if [ ${loopCount} -gt ${timeout} ] ; then
            echo "${LOG_ERROR} fluxbox failed to start."
            exit 1
        fi
    done
}
...
run_vnc_server() {
    ...
    if [ -n "${VNC_SERVER_PASSWORD}" ] ; then
        ...
        if ! x11vnc -storepasswd "${VNC_SERVER_PASSWORD}" "${passwordFilePath}" ; then
            echo "${LOG_ERROR} Failed to store x11vnc password."
            exit 1
        fi
        passwordArgument=-"-rfbauth ${passwordFilePath}"
        echo "${LOG_INFO} The VNC server will ask for a password."
    else
        echo "${LOG_WARN} The VNC server will NOT ask for a password."
    fi
